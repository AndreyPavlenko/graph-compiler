    ...
    if (!size) {
      ...
      for (unsigned i = 0, n = srcType.getRank(); i < n; i++) {
        numElements = LLVM::MulOp::create(
            rewriter, loc, numElements, srcDsc.size(rewriter, loc, i));
      }
    ...
    for (auto arg : kernelArgs) {
      if (auto type = gpuLaunch.getKernelOperand(i++).getType();
        ...
        auto ptr = LLVM::AllocaOp::create(
            rewriter, loc, helper.ptrType, typeConverter->convertType(type),
    ...
    auto array = [&](SmallVector<Value> &values) {
      ...
      for (size_t i = 0, n = values.size(); i < n; i++) {
        auto elementPtr = LLVM::GEPOp::create(
            rewriter, loc, helper.ptrType, helper.idxType, arrayPtr,
...
struct GpuToGpuOcl final : gc::impl::GpuToGpuOclBase<GpuToGpuOcl> {
  ...
  void runOnOperation() override {
    ...
    for (auto &name : helper.kernelNames) {
      ...
      assert(ptr);
      auto ptrVal = LLVM::LoadOp::create(
          rewriter, loc, helper.ptrType, LLVM::AddressOfOp::create(rewriter, loc, ptr));
